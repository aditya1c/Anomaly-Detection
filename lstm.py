...
def file_load_np_array(file_name):
...
stop_range=3600000
if training==True:
    file_loc=['input/rw'+rw_id+'inp.txt','input/rw'+rw_id+'dfc.txt']
    data=file_load_np_array(file_loc[1])
    full_seq,full_lab=create_inout_sequences(data[start_range:stop_range,-1],seq_len)
    st_epoch = 0
...
#     test_lab=full_lab[val_pos:]
    ...
    if load==True:
        checkpoint = torch.load("output/checkpointrw"+rw_id+".tar")
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        st_epoch = checkpoint['epoch']
        bt_m = torch.load("output/best_model_trend_presentrw"+rw_id+".tar")
        best_loss = bt_m['loss']
        if decay ==True:
            lr=lr*0.1
    ...
    # print(loss,epoch)
        ...
        file = open('output/lossrw'+rw_id+'.txt',mode='r')
        all_of_it = file.read()
        file.close()

        file = open('output/lossrw'+rw_id+'.txt',mode='a')
    ...
    for i in range(st_epoch,epochs):    
        ...
        print(f'epoch: {i:3} train loss: {loss:10.8f}')
        val_loss=val(model,val_seq,val_lab,loss_function)
        print(f'epoch: {i:3} val loss: {val_loss:10.8f}')
        if i==0:
          best_loss=val_loss
          save_model(model,optimizer,i,best_loss,"best_model_trend_presentrw"+rw_id+".tar")
          ...
          lr=lr*0.05
          optimizer=decay_lr(optimizer,lr)
        save_model(model,optimizer,i,val_loss,"checkpointrw"+rw_id+".tar")
        f = open("output/lossrw"+rw_id+".txt", "a")
        f.write(str(i)+" "+str(loss)+" "+str(val_loss)+"\n")
        f.close() 

    print(f'epoch: {i:3} train loss: {loss:10.10f}')
    val_loss=val(model,val_seq,val_lab,loss_function)
    print(f'epoch: {i:3} val loss: {val_loss:10.10f}')


if training==False:
    file_loc=['input/testRw'+rw_id+'TorqDfc.txt','input/rw'+rw_id+'dfc.txt']
    data=file_load_np_array(file_loc[1])[0:1700000]
    test_seq,test_lab=create_inout_sequences(data[:,-1],seq_len)
        
    checkpoint = torch.load("output/best_model_trend_presentrw"+rw_id+"_v2.tar")
    model.load_state_dict(checkpoint['model_state_dict'])
    ...
    np.savetxt('output/ana_scorerw'+rw_id+'.txt',ana_score)
    np.savetxt('output/predrw'+rw_id+'.txt',pred)
